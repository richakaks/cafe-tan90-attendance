...
- `appscript/Code.gs` — server-side logic: employee roster (add/rename/remove, up to 8, tracked by a stable slot id so a rename or removal never loses or splits history, each with their own PIN), standing/recurring tasks (auto-seeded onto every matching day), monthly spreadsheet auto-creation (one Google Sheet per calendar month, a tab per day, pre-built in advance, with native checkboxes instead of raw TRUE/FALSE on the attendance status columns — applied retroactively to already-built tabs too), attendance log-in/out and task assignment + completion tracking (both gated by per-employee PIN so nobody can act on someone else's behalf, with the admin PIN working everywhere as a master override), same-day self-undo of an employee's own punch or task-done tap, admin PIN check, admin correction of a specific log-in/log-out or task-done time for today or any past day (attendance edits are sticky-flagged as "edited" per field once touched; task-done edits are not, a deliberate simplification), personal payroll lookups scoped to one employee (day and month, same computation as the admin view), and payroll (per-employee hourly rate history with effective dates, exact-to-the-second hours from log-in/out, day and browsable month totals).
- `appscript/Staff.html` — the single page that's actually served (`doGet()` always renders this one). Has the shared-screen Staff tab — an identify-yourself step (name + PIN) that unlocks just that person's own row (everyone else's stays visible but greyed out and un-tappable), log in/out, mark tasks done, same-day self-undo, a "My hours & pay" view of your own attendance/payroll history, self-service PIN change, and a "Switch user" button so a shared or borrowed device never stays signed in as the last person — and pulls in `Admin.html` as its Admin tab via Apps Script's `include()` — one URL, two tabs, no separate admin link to remember.
- `appscript/Admin.html` — **not served on its own** — a body-only partial included into `Staff.html`'s Admin tab. PIN-gated (the tab is visible to anyone, but nothing behind it renders until the PIN is entered), split into two sub-tabs: **Tasks** (one unified task list for any date, each task markable "Repeats" on chosen days of the week right on its own row and reorderable by dragging its ≡ handle, a "Plan tasks ahead" calendar for one-off tasks across a week or month, a per-employee done-time editor on each assigned task, employee management including per-person PIN set/reset, and a shortcut to open the live Sheet) and **Payroll** (set each employee's hourly rate — with an effective date so a raise can start today, be backdated, or be scheduled ahead — browse exact-hours pay for any day, including log-in/log-out time columns, or any month past or present, and correct or clear a person's log-in/log-out time for any day up to today when they missed a punch or logged the wrong time). Typing into any task or employee name field auto-capitalizes each word as you go.
...
- `preview.html` — a standalone, in-browser mockup (no backend) of both pages — including the identify/lock/undo/My-hours flow and admin PIN management — useful for reviewing the design/flow without deploying anything.
